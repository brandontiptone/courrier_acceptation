...
with col1:
    ...
    subvention = st.number_input("Subvention globale (€)", min_value=0, value=0, step=100)
...
def generer_pdf(date_dossier, adresse, subvention, prime, tva):
    ...
    mois_str = date_dossier.strftime("%B %Y")
    ...
    story.append(Paragraph(f"Le {date_str}", style_salutation))
    story.append(Spacer(1, 0.5*cm))
